...
export default function NonSsl() {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const response = await fetch('/api/non-ssl', {
                method: 'GET',
                headers: {
            ...
            <div className="w-full max-w-6xl mx-auto md:px-6 py-2">
                <div className="px-5 xl:px-0 mx-auto">
                    <h1
                        ...
                        data-aos="fade-up"
                    >
                        <Balancer>Non SSL <span className='bg-clip-text text-transparent bg-[linear-gradient(to_right,theme(colors.green.300),theme(colors.green.100),theme(colors.sky.400),theme(colors.yellow.200),theme(colors.sky.400),theme(colors.green.100),theme(colors.green.300))] bg-[length:200%_auto] animate-gradient'>Lavalink</span></Balancer>
                    </h1>
                    <p
